...
class NN(nn.Module):
    def __init__(self, nOUT):
        ...
        # Assuming nOUT remains the same as when it was pre-trained
    ...
    def forward(self, x, l):
        ...
        # --- NEW: Apply both poolings ---
        #_max = self.pool_max(x).squeeze(2) # Shape: [Batch, 256]
        x_avg = self.pool_avg(x).squeeze(2) # Shape: [Batch, 256]
        
        # --- NEW: Concatenate features ---
        # Combines the max features, average features, and the external leads 'l'
        #x = torch.cat((x_max, x_avg, l), dim=1)
        x = torch.cat((x_avg, l), dim=1)
        x = self.fc_1(x)
