...
def find_thresholds(filename,model_directory):
    # Pickle serializies data -> it is store on row by another
    ...
    for j in range(N):
        prc, rec, thr = precision_recall_curve(y_true=t[:, j], y_score=y[:, j]) ############ upraveno z probas_pred na y_score
...
class mytqdm(tqdm):
    ## Print loss calc and progress bar
...
class challengeloss(nn.Module):
    def __init__(self):
        ...
        self.weights = torch.from_numpy(self.weights).float().to(DEVICE).requires_grad_(False)
...
def _training_code(data_directory, model_directory, ensamble_ID):
    ...
    for epoch in range(EPOCHS):
        ...
        writer.add_scalar("AUPRC/valid", valid_auprc, epoch)
