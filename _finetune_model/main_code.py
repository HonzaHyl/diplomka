...
CONFIG = {
    ...
    "unfreeze_epoch": 5, # NEW: The epoch where Stage 2 begins
    
    # STAGE 1: Only the new fully connected layer learns
    "layer_tuning_stage_1": {
        "conv": {"trainable": False, "lr": 0},
        "bn":   {"trainable": False, "lr": 0},
        "rb_0": {"trainable": False, "lr": 0},
        "rb_1": {"trainable": False, "lr": 0},
        "rb_2": {"trainable": False, "lr": 0},
        "rb_3": {"trainable": False, "lr": 0},
        "rb_4": {"trainable": False, "lr": 0},
        "fc_1": {"trainable": True,  "lr": 1e-4} # Head warms up
    },
    
    # STAGE 2: Top layers of the backbone wake up gently
    "layer_tuning_stage_2": {
        "conv": {"trainable": False, "lr": 0},
        "bn":   {"trainable": False, "lr": 0},
        "rb_0": {"trainable": False, "lr": 0},
        "rb_1": {"trainable": False, "lr": 0},
        "rb_2": {"trainable": True, "lr": 3e-7},
        "rb_3": {"trainable": True,  "lr": 3e-7}, # Unfrozen
        "rb_4": {"trainable": True,  "lr": 3e-7}, # Unfrozen
        "fc_1": {"trainable": True,  "lr": 3e-5}
    }
}
...
class FocalLoss(nn.Module):
    ...
    # FIX: Changed 'inputs' to 'input' and 'targets' to 'target'
    def forward(self, input, target):
        # Calculate standard Cross Entropy Loss (with class weights)
...
def _training_code(data_directory, model_directory, ensamble_ID, resume_checkpoint=None):
    # === TensorBoard setup ===
    ...
    # replacement=True is required so it can reuse minority class windows
    ...
    if resume_checkpoint and os.path.exists(resume_checkpoint):
        ...
        if start_epoch >= CONFIG["unfreeze_epoch"]:
            CONFIG["layer_tuning"] = CONFIG["layer_tuning_stage_2"]
        else:
            CONFIG["layer_tuning"] = CONFIG["layer_tuning_stage_1"]
        ...
        # scheduler.load_state_dict(checkpoint['scheduler_state_dict']) # <-- TEMPORARILY DISABLED: Restart the StepLR schedule!
    ...
    for epoch in range(start_epoch, EPOCHS):
        ...
        if epoch == CONFIG["unfreeze_epoch"]:
            print("\n🚀 INITIATING STAGE 2: Unfreezing backbone layers...")
            
            # Switch the active configuration to Stage 2
            CONFIG["layer_tuning"] = CONFIG["layer_tuning_stage_2"]
            
            # Rebuild the optimizer so it registers the newly trainable parameters
            # and resets momentum states for the unfrozen layers
            opt = build_flexible_optimizer(model, CONFIG)
            
            # Rebuild the scheduler so it tracks the new optimizer
            scheduler = optim.lr_scheduler.StepLR(opt, step_size=CONFIG["step_size"], gamma=CONFIG["gamma"])

        
        # Unpack the new train loss
        train_loss, train_auprc, train_auroc, train_f1, train_cm = train_part(model, train, opt, loss_fn)
...
def valid_part(model, dataset, loss_fn): # <-- Added loss_fn
    ...
    with torch.no_grad():  
        for (x, t, l) in dataset:
            sig_len = x.shape[-1]
            window_size = WINDOW_SIZE # Or your specific window size
            step_size = STEP_SIZE
            ...
            patient_p = torch.stack([prob_negative, mean_prob_positive], dim=1)
            
            # --- NEW: Calculate Validation Loss ---
            # To calculate CrossEntropyLoss, we need the raw logits. 
            # We approximate patient-level logits by taking the log of the mean probabilities.
            # Adding a tiny epsilon prevents log(0) errors.
            patient_logits = torch.log(patient_p + 1e-8)
            t_indices = torch.argmax(t, dim=1)
            
            batch_loss = loss_fn(input=patient_logits, target=t_indices)
    ...
    avg_valid_loss = total_loss / num_batches # <-- Calculate average loss
    
    # Return the loss alongside the metrics
    return avg_valid_loss, auprc, auroc, f1, cm, targets, outputs, best_threshold
