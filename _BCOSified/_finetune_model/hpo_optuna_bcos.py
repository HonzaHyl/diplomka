...
MODEL_DIR = "/srv/home/jhyl/Afib_recurrence/diplomka/results/hpo_runs_bcos"
N_TOTAL_TRIALS = 50
K_FOLDS = 4
N_GPUS = 3
# -------------------------
...
def objective(trial, device):
    ...
    head_wd      = trial.suggest_float("head_weight_decay", 1e-3, 0.5, log=True) # Usually higher WD for the head
    ...
    with mlflow.start_run(run_name=f"Trial_{trial.number}", nested=True):
        ...
        try:
            ...
            # Extract score from the fold results (best_auroc, train_at_best)
            ...
            # Return metrics for Optuna (Multi-objective optimization)
...
def run_optimize(device, study_name, storage_name, n_trials):
    # Each process loads the study and runs its own subset of trials
...
if __name__ == "__main__":
    ...
    mlflow.set_experiment("BCOSified_finetuning_HPO")
    
    study_name = "bcos_finetune_hpo_v1.0"
    ...
    study = optuna.create_study(
        ...
        directions=["maximize", "minimize"], # Maximize Mean AUROC, Minimize Std Dev
        load_if_exists=True,
    )
    ...
    # Multiprocessing setup (using 'spawn' for CUDA safety)
    ...
    print("\n" + "="*30)
    print("PARALLEL B-COS HPO COMPLETE")
